...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FE] text-slate-900 pb-24 font-sans select-none overflow-x-hidden">
      {/* Header */}
          ...
          <div className="space-y-4">
            ...
            {filteredTransactions.map(t => (
              <div key={t.id} className="bg-white p-4 rounded-2xl border border-slate-50 flex items-center shadow-sm group active:scale-95 transition-transform">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center mr-4 ${t.type === 'income' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                ...
                <div className="text-right flex items-center gap-3">
                  ...
                  <button onClick={() => handleDeleteTransaction(t.id)} className="text-slate-300 hover:text-rose-500">
                    <Trash2 size={16}/>
                  </button>
                </div>
              </div>
                ...
                {plans.map(p => (
                  <div key={p.id} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden group">
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-2">
                        <div className={`p-2 rounded-lg ${p.type === 'income' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                          <Clock size={16}/>
                        </div>
                        <div>
                          <p className="font-bold text-slate-800 text-sm">{p.title}</p>
                          <p className="text-[10px] font-bold text-slate-400 uppercase">{p.category} • {p.date}</p>
                        </div>
                      </div>
                      <p className={`font-black text-sm ${p.type === 'income' ? 'text-emerald-600' : 'text-rose-600'}`}>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button 
                        ...
                        className="w-10 h-10 flex items-center justify-center bg-slate-50 text-slate-400 rounded-xl hover:text-rose-500"
                      >
                        <Trash2 size={16}/>
                      </button>
                    </div>
      ...
      {/* Bottom Tab Bar */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-t border-slate-100 px-6 pb-8 pt-3 flex justify-between items-center z-40">
        ...
        <div className="relative -top-8">
          <button 
            onClick={() => { setModalMode('transaction'); setShowAddModal(true); }}
      ...
      {/* Add Transaction/Plan Modal */}
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-end animate-in fade-in duration-300">
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={() => setShowAddModal(false)}></div>
          <div className="relative w-full bg-white rounded-t-[3rem] p-8 pb-12 animate-in slide-in-from-bottom-full duration-500">
            <div className="w-12 h-1.5 bg-slate-200 rounded-full mx-auto mb-8"></div>
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-black italic tracking-tighter uppercase">{modalMode === 'transaction' ? 'GHI CHÉP MỚI' : 'LÊN LỊCH MỚI'}</h2>
